...
// const final2014 = fifaData.filter(arama(item));
...
console.log(
  fifaData.filter((obj) => obj.Year === 2014 && obj.Stage === "Final")[0][
    "Away Team Name"
  ]
);
...
function Finaller(fifa) {
  const finals = fifa.filter((obj) => obj.Stage === "Final");
  return finals;
}
...
function Yillar(fifa, callback) {
  const finals = callback(fifa);
...
function Kazananlar(fifa, callback) {
  const finaller = callback(fifa);
  let kazananlar = [];
